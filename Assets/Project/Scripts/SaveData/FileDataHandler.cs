...
public class FileDataHandler
{
    ...
    public GameData Load(string profileId, bool allowRestoreFromBackup = true) 
    {
        ...
        if (File.Exists(fullPath)) 
        {
            ...
            {
                loadedData.scene = loadedData.GetSceneData(currentSceneName);
                if(loadedData.scene == null)
                {
                    //First time this scene loading
                    loadedData.CreateNewSceneData();
                }

                //Debug.Log("FFFFFile scene loaded: "+currentSceneName);
            }
            else
            {
                Debug.LogError("Error occured when trying to get current scene name");
            }
        }
    ...
    public string GetMostRecentlyUpdatedProfileId(/*out GameData lastGameData*/) 
    {
        ...
        foreach (KeyValuePair<string, GameData> pair in profilesGameData) 
        {
            ...
            {
                DateTime mostRecentDateTime = DateTime.FromBinary(profilesGameData[mostRecentProfileId].globals.lastUpdated);
                DateTime newDateTime = DateTime.FromBinary(gameData.globals.lastUpdated);
